...
export class Roles {
  ...
  async createRole(role) {
    ...
    try {
      id = await this.roles.insert({'name': role.trim()})
      return id
    } catch (e) {
  ...
  async deleteRole(role) {
    ...
    var foundExistingUser = await this.users.findOne(
      {roles: {$in: [role]}},
      {fields: {_id: 1}})
    ...
    var thisRole = await this.roles.findOne({name: role})
    if (thisRole) {
      await this.roles.remove({_id: thisRole._id})
    }
  }
   ...
   */
  async addUsersToRoles(users, roles, group) {
    // use Template pattern to update user roles
    await this._updateUserRoles(users, roles, group, this._update_$addToSet_fn.bind(this))
  }
   ...
   *                         included in checks for any group.
   */
  async setUserRoles(users, roles, group) {
    // use Template pattern to update user roles
    await this._updateUserRoles(users, roles, group, this._update_$set_fn.bind(this))
  }
   ...
   */
  async removeUsersFromRoles(users, roles, group) {
    var update
   ...
   */
  async userIsInRole(user, roles, group) {
    var id,
      userRoles,
      query,
      groupQuery,
    ...
    found = await this.users.findOne(query, {fields: {_id: 1}})
   ...
   */
  async getRolesForUser(user, group) {
    if (!user) return []
    if (group) {
   ...
   */
  async getAllRoles() {
    return await this.roles.find({}, {sort: {name: 1}}).toArray()
  }
   ...
   */
  async getUsersInRole(role, group, options) {
    var query,
      roles = role,
      groupQuery
   ...
   */
  async getGroupsForUser(user, role) {
    var userGroups = [];
   ...
   * @return {Object} update object for use in MongoDB update command
   */
  _update_$addToSet_fn(roles, group) {
    var update = {}
   ...
   *   @param {String} [group]
   */
  async _updateUserRoles(users, roles, group, updateFactory) {
    if (!users) throw new Error ("Missing 'users' param")
    if (!roles) throw new Error ("Missing 'roles' param")
    if (group) {
      if ('string' !== typeof group)
    ...
    existingRoles = _.reduce(await this.roles.find({}).toArray(), function (memo, role) {
      memo[role.name] = true
      return memo
    }, {})
    await _.map(roles, async (role) => {
